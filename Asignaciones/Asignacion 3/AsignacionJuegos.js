const videojuegos = Object.freeze([
    ...
    1.
    const juegosDivisiblesPor3 = videojuegos.filter(juego => juego.id % 3 === 0);
    
    console.log(juegosDivisiblesPor3);
    ...
    3.
    const juegosConMasDeUnGenero = videojuegos.filter(juego => juego.genero.includes("-"));

    console.log(juegosConMasDeUnGenero);
    ...
    const juegosConIdSuperiorA19 = videojuegos.filter(juego => juego.id > 19).map(juego => juego.nombre);

    console.log(juegosConIdSuperiorA19);
    ...
    7.
    const primerGeneroDeJuegosAventura = videojuegos.filter(juego => juego.genero.split("-")[1] === "aventura").map(juego => juego.genero.split("-")[0]);

    console.log(primerGeneroDeJuegosAventura);
    ...
    9.
    const juegosNoMultiplosDe5 = videojuegos.filter(juego => juego.id % 5 !== 0).map(juego => juego.nombre);

    console.log(juegosNoMultiplosDe5);


    10.
    const videojuegosModificados = videojuegos.map(juego => juego.id === 5 ? { ...juego, genero: "otro" } : juego);

    console.log(videojuegosModificados);

    11. 
    const videojuegosOrdenados = videojuegos.sort((a, b) => a.nombre.localeCompare(b.nombre));

    console.log(videojuegosOrdenados);
